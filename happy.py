...
class Config(object):
    ...
    def __init__(self, cfg_id, date=None, overrides=None):
        ...
        self.ns = self._dict_to_namespace(config_data)
    ...
    @staticmethod
    def _dict_to_namespace(data_dict):
        """Recursively convert a dictionary to a namespace."""
        namespace = CustomNamespace()
        for key, value in data_dict.items():
            if isinstance(value, dict):
                nested_namespace = Config._dict_to_namespace(value)
                setattr(namespace, key, nested_namespace)
            else:
                setattr(namespace, key, custom_parser(str(value)))
        return namespace
    ...
    def __getattr__(self, name):
        # This method is called when the default attribute access fails with an AttributeError
        return None
